using Hypha.ActivationFunctions;
using Hypha.Operations;

namespace Hypha.Core.Tests.Operations;

public class InputLayerOperationTests
{

    private readonly InputLayerOperation inputLayerOperation;

    public InputLayerOperationTests() => inputLayerOperation = new InputLayerOperation(new Normalization());

    [Theory]
    [InlineData(new double[] { 33, 12, 10, 1, 12, 12, 11, 11 })]
    [InlineData(new double[] { 0, 0, 0, 0, 0, 0, 0, 0, 0 })]
    public void Forward_Input_Success(double[] input)
    {
        var output = inputLayerOperation.Forward(input);

        Assert.All(output, x => Assert.True(x <= 1));
    }
}
